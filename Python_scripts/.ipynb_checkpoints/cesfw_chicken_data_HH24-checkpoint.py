...
#Get file name
file_name = "preprocessing_larry_chicken_HH24.h5ad"
...
print(f"[ENV] Host: {platform.node()}, Python: {platform.python_version()}")
print(f"[ENV] SLURM_CPUS_PER_TASK={os.getenv('SLURM_CPUS_PER_TASK')}, "
...
# **NEW: Input matrix stats**
print(f"[cESFW] Input matrix for ESFW: {adata_counts.shape[0]} cells × {adata_counts.shape[1]} genes")
approx_bytes = adata_counts.shape[0] * adata_counts.shape[1] * 8  # float64
print(f"[cESFW] Dense matrix payload ≈ {approx_bytes/1e9:.2f} GB (matrix only)")
...
ESSs, EPs = cESFW.Parallel_Calculate_ESS_EPs(os.path.join(base_path , "cesfw" ,folder_name) + "/" , Use_Cores=32)
...
# **NEW: Final summary**
print(f"[cESFW] Done! Saved ESSs and EPs to {folder_path}")
print(f"[cESFW] Final matrix size was {adata_counts.shape[0]} cells × {adata_counts.shape[1]} genes")