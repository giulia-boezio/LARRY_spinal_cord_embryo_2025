...
This notebook generates figures for Figure 4 and Supplementary Figure S5, covering dI4/dI5 neuron identity, clonal sharing, and proportional dynamics across timepoints (HH28, HH31, HH35) in the injHH12-collHH28/31/35 chicken spinal cord LARRY dataset.

**Data required:**
- `preprocessing_larry_chicken_clusters_newwave.rds` — main integrated Seurat object
- `dI4_dI5_Propotions.csv` — mouse dI4/dI5 cell counts per embryonic stage (Delile 2019)

**Outputs:**
- `dI4_dI5_UMAP-dim2-3.png` — UMAP highlighting dI4 and dI5 clusters
- `pie_dI4-5_neurons.pdf` — pie chart of dI4/dI5 proportion among all neurons
- `circos_clonal_dI4_dI5.pdf` — circos/chord diagram of clonal co-occurrence between neuronal subtypes
- `dI4_dI5_ratio_LARRY_median_replicates.pdf` — stacked bar of median dI4/dI5 ratios in LARRY+ cells
- `dI4_dI5_ONLY_clone_size-NEW-all-other-cells-included.pdf` — clone size distribution for dI4-only vs shared clones
- `dI4_dI5_KDE_experiments.pdf` — KDE/histogram of dI4 fraction within clones per timepoint
- `volcano-DEGs-pval.pdf` — volcano plot of DEGs between dI4 shared-with-dI5 vs dI4-only cells
- `mouse_dI4_dI5_ratio_onlyE11.pdf` — stacked bar of dI4/dI5 ratios in mouse (S5)
- `KCNIP4_across_exp_vlnplot.pdf` — violin plot of KCNIP4 expression in dI4 cells across timepoints
...
## Paths
```{r paths}
# Set output paths for plots and data
...
# Load integrated Seurat object and add experiment metadata columns
...
## dI4/dI5 UMAP
```{r di4-di5-umap}
# Highlight dI4_dILA and dI5_dILB clusters on UMAP (dims 2–3)
...
## Percentage dI4/dI5 over all neurons
```{r di4-di5-pie-proportion}
# Pie chart showing proportion of dI4/dI5 among all neuronal subtypes
library(dplyr)
library(ggplot2)
...
## Circos plot dI4/dI5 clonal co-occurrence
```{r circos-clonal-matrix-build}
# Build symmetric clonal co-occurrence matrix across neuronal subtypes for chord diagram
library(tidyverse)
library(Seurat)
...
```{r circos-plot-render}
# Render and save circos chord diagram highlighting dI4–dI5 clonal links
...
## dI4/dI5 ratio stacked bar (median per replicate)
```{r di4-di5-ratio-stacked-bar}
# Compute per-replicate dI4/dI5 ratios in LARRY+ cells and plot stacked median bars across timepoints
...
## Clone size for dI4-only vs shared clones

### Build clone classification table
```{r di4-di5-clone-classification}
# Classify multicellular clones as dI4-only, dI5-only, shared dI4+dI5, dpL-only, or Other
...
### Clone size boxplot by class
```{r di4-di5-clone-size-boxplot}
# Plot clone size distribution for dI4-only vs dI4+dI5 shared clones, faceted by timepoint
...
## Distribution of dI4/dI5 ratio within clones
```{r di4-di5-ratio-kde}
# KDE and histogram of the dI4 fraction within clones containing dI4 and/or dI5, per timepoint
...
## Divide dI4 by clonal sharing status

### Classify dI4 cells as shared-with-dI5 or dI4-only
```{r di4-clone-category-assignment}
# Annotate each dI4_dILA cell as belonging to a clone shared with dI5 or not
...
### Add dI4 clone category to Seurat object and subset
```{r di4-subset-seurat}
# Add dI4_clone_category metadata column and subset Seurat object to dI4 cells only
...
## Calculate DEGs between shared-with-dI5 and dI4-only cells
```{r di4-deg-volcano}
# Run FindMarkers on dI4 subset and plot volcano of shared-with-dI5 vs dI4-only DEGs
...
### Mouse dI4/dI5 proportions across embryonic stages
```{r mouse-di4-di5-proportions}
# Stacked bar chart of dI4/dI5 ratios in mouse spinal cord at E11.5–E13.5 (Delile 2019)
library(dplyr)
library(ggplot2)
library(readr)
...
### Gene expression across timepoints (KCNIP4)
```{r kcnip4-expression-violin}
# Violin plot of KCNIP4 expression in dI4_dILA cells across HH28/HH31/HH35
library(ggpubr)
