...
### Set up variables
```{r modelling-setup}
# Defines observed clone counts and binomial model probabilities for 3-cell clone analysis
...
```{r barplot-observed-vs-binomial}
# Plots observed clone counts against binomial expected values as a bar chart with overlaid points (Fig. 6I)
...
# Runs chi-square goodness-of-fit test and binomial test for enrichment of the 1.0 ratio bin
...
```{r mixed-model-setup}
# Re-defines observed counts and binomial probabilities as inputs for the mixture model
...
```{r fit-fixed-mixture-model}
# Fits a fixed mixture model (alpha * binomial + (1-alpha) * dI4-only) and optimises alpha by MLE
...
```{r alpha-profile-loglik}
# Computes the profile log-likelihood across alpha values and plots the MLE with 95% CI (Fig. S9A)
...
```{r chisq-statistics}
# Computes chi-square goodness-of-fit statistics for binomial and fixed mixture model fits
...
### Bootstrapping
```{r bootstrapping}
# Generates 10,000 bootstrap resamples from observed data, binomial model, and fixed mixture model
...
#### Bootstrap chi-square and empirical p-values
```{r bootstrap-chisq-pvalues}
# Computes bootstrap chi-square distributions and empirical p-values for each model, then saves fit summary
...
# Computes log-likelihood, AIC, and BIC for binomial and fixed mixture models, then saves to CSV
...
```{r plot-chisq-distributions}
# Plots bootstrap chi-square distributions for binomial and fixed models with observed chi-square overlaid (Fig. S9B)
...
#### Summarise bootstrap matrix to mean and differences
```{r bootstrap-summary-table}
# Summarises bootstrap distributions (mean, 95% CI) for each model and saves a clean comparison table to CSV
...
```{r bootstrap-boxplots}
# Plots side-by-side boxplots of bootstrapped clone count distributions for observed and fixed mixture model
