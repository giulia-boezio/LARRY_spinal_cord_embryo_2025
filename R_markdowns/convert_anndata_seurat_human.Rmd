...
This notebook converts the exported AnnData matrices from Python preprocessing into a normalised Seurat object for the human spinal cord LARRY dataset, so that downstream R analyses can use a single RDS file rather than raw flat files.

**Data required:**

- `preprocessing_larry_human_matrix.mtx` — sparse count matrix (cells × genes)
- `preprocessing_larry_human_genes.csv` — gene names
- `preprocessing_larry_human_cells.csv` — cell barcodes
- `preprocessing_larry_human_meta.csv` — per-cell metadata (includes `cell_type`, etc.)
- `preprocessing_larry_human_umap.csv` — pre-computed 3-D UMAP coordinates

**Outputs:**

- `preprocessing_larry_human_clusters.rds` — normalised, scaled Seurat object with UMAP embedding and metadata attached
